...
# Calculate arithmetic density for all years
wp_df_melted = wp_df_melted.merge(wp_df[['ISO-3', 'Country', 'Area (km²)']], on=['ISO-3', 'Country'], how='left')
wp_df_melted['Arithmetic Density'] = wp_df_melted['Population'] / wp_df_melted['Area (km²)']

# Create the choropleth map
fig = px.choropleth(wp_df_melted, locations='ISO-3', color='Arithmetic Density', locationmode="ISO-3", scope="world", width=1200, height=600, title='World Arithmetic Density by Country (1970-2110)', hover_name="Country", hover_data={'ISO-3': False,}, projection='equirectangular', range_color=(0, 500), animation_frame='Year', color_continuous_scale=px.colors.sequential.Viridis)
